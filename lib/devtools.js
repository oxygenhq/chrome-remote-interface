...
function List(options, callback) {
    ...
    devToolsInterface(options, (err, tabs) => {
        if (err) {
            ...
            // Fix for https://github.com/electron/electron/issues/11524
            // Happens on Windows 7, OEM CP 862, A CP 1255. when application is execute from network drive
            tabs = tabs.replace(/[^A-Za-z 0-9 \.,\?""!@#\$%\^&\*\(\)-_=\+;:<>\/\\\|\}\{\[\]`~]*/g, '');
